...
contract ybETH_RedeemTest is ybETH_BaseTest {
  ...
  function testCorrectness_WhenRedeemSuccessfully() external {
    ...
    uint256 _receivedWeth = ybeth.redeem(500 ether, address(this), address(this));
    // Assert
    assertEq(ybeth.balanceOf(address(this)), 500 ether);
    assertEq(ybeth.totalAssets(), 2_040.1 ether - _receivedWeth);
    assertEq(weth.balanceOf(address(ybeth)), 2_040.1 ether - _receivedWeth);
    assertEq(ybeth.totalSupply(), 1_000.1 ether + _expectedAliceShares - 500 ether);
    assertEq(weth.balanceOf(address(this)), _receivedWeth);

    // The 1st user redeem the rest.
    _receivedWeth = ybeth.redeem(500 ether, address(this), address(this));
    // Assert
    assertEq(ybeth.balanceOf(address(this)), 0);
    assertApproxEqAbs(ybeth.totalAssets(), 2_040.1 ether - _receivedWeth * 2, 1);
    assertApproxEqAbs(weth.balanceOf(address(ybeth)), 2_040.1 ether - _receivedWeth * 2, 1);
    assertEq(ybeth.totalSupply(), _expectedAliceShares + 0.1 ether);
    assertApproxEqAbs(weth.balanceOf(address(this)), _receivedWeth * 2, 1);
